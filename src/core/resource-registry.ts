...
export class ResourceRegistry {
  ...
  private parseResource(filePath: string, source: 'bundled' | 'user', inferredType?: string): Resource | null {
    ...
    // Infer type from directory if not specified (e.g., files in runbooks/ are runbooks)
    if (!metadata.type && inferredType) {
...
export function createRegistry(userTemplatesPath?: string, userRunbooksPath?: string): ResourceRegistry {
  ...
  // From lib/core/resource-registry.js → package root
  // Go up two levels: lib/core/ → lib/ → package root
